...
from overlay_odom import (BAG, VIDEO, EXTR, REC, ODOM_TOPIC, VIDEO_START_EPOCH,
                          load_extrinsics, load_odom, nearest)


def pick_frames(ts, pos, fps, n, offset, zmin=0.3):
    ...
    for si in sorted(sel):
        fi = int(round((ts[si] - VIDEO_START_EPOCH - offset) * fps))
        out.append((fi, si))
    return out
...
def collect_clicks(frames, ts, pos, K, dist, rvec, tvec):
...
def main():
    ...
    ap.add_argument("--out", default=os.path.join(os.path.dirname(EXTR),
                                                  "webcam_extrinsics_clicked.json"))
    ...
    if args.clicks:
        ...
        picks = pick_frames(ts, pos, fps, args.n, args.offset)
        print(f"picked {len(picks)} frames; opening GUI...")
        corr = collect_clicks(picks, ts, pos, K, dist, rvec0, tvec0)
        if len(corr) < 4:
    ...
    def solve_at(off):
        objp = np.array([pos_at(VIDEO_START_EPOCH + f / fps + off) for f in frames],
                        np.float64)
    ...
    rms_before = reproj_rms(np.array([pos_at(VIDEO_START_EPOCH + f / fps) for f in frames]),
    ...
    json.dump(E, open(args.out, "w"), indent=2)
    print(f"\nsaved -> {args.out}")
    print(f"verify: python3 overlay_odom.py --extr {os.path.basename(args.out)}")
